...
cellInitializerBit :: ∀ n x t. (KnownNat n, KnownNat x, KnownFloating t) => Gen (DenseP t (n + x) n)
...
instance (KnownNat n, KnownNat x, KnownFloating t) => KnownTensors (LSTMP t n x) where
  travTensor f s (LSTMP x y z w) = LSTMP <$> travTensor f (s<>"_f") x <*> travTensor f (s<>"_i") y <*> travTensor f (s<>"_c") z <*> travTensor f (s<>"_o") w
instance (KnownNat n, KnownNat x, KnownFloating t) => ParamWithDefault (LSTMP t n x) where
...
-- | Standard LSTM
lstm :: ∀ n x t. KnownNat x => KnownNat n => KnownFloating t
...
instance (KnownNat n, KnownNat x, KnownFloating t) => KnownTensors (GRUP t n x) where
  travTensor f s (GRUP x y z) = GRUP <$> travTensor f (s<>"_z") x <*> travTensor f (s<>"_r") y <*> travTensor f (s<>"_w") z
instance (KnownNat n, KnownNat x, KnownFloating t) => ParamWithDefault (GRUP t n x) where
  defaultInitializer = GRUP <$> (denseWeights <$> cellInitializerBit) <*> (denseWeights <$> cellInitializerBit) <*> (denseWeights <$> cellInitializerBit)



-- | Standard GRU cell
gru :: ∀ n x t. KnownNat x => (KnownNat n, KnownFloating t) => GRUP t n x ->
...
defStackP :: KnownNat n => KnownFloating w => Gen (StackP w n)
...
instance (KnownNat n, KnownFloating w) => (ParamWithDefault (StackP w n)) where
...
-- depth of the stack.
stackRU :: ∀k n bs w. KnownNat k => KnownNat n => (KnownNat bs) => (KnownFloating w) => StackP w n ->
